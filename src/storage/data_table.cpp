...
SharedPtr<DataTable> DataTable::MakeSummaryResultTable(u64 count, u64 sum) {
    ...
    column_defs.emplace_back(MakeShared<ColumnDef>(0, MakeShared<DataType>(LogicalType::kBigInt, nullptr), "count", std::set<ConstraintType>()));
    column_defs.emplace_back(MakeShared<ColumnDef>(1, MakeShared<DataType>(LogicalType::kBigInt, nullptr), "sum", std::set<ConstraintType>()));
